...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2) Check if the user exists against the email
  const user = await User.findOne({ email }).select('+password');

  // 3) If everything is correct, retrun the token
...
exports.updatePasswrod = catchAsync(async (req, res, next) => {
  // 1) Get user form collection
  const user = await User.findById(req.user.id).select('+password');

  // 2) Check if POSTED current passwrod is correct
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError('Your currnet password is wrong.', 401));
  }

  // 3) Else update the password
  user.password = req.body.password;
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
